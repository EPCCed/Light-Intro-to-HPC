# Comparing your laptop to a supercomputer

One of the things we’ve tried to emphasise when discussing supercomputer hardware is the similarities to your laptop.
...
In the previous step you have seen a deconstructed laptop, in this video David takes apart a compute blade from the [HECToR](http://www.hector.ac.uk/) supercomputer (predecessor to ARCHER which is the predescessor for ARCHER2!).
...
In this part we have introduced shared and distributed memory architectures. These are two different approaches of putting individual CPU-cores together to form large parallel machines. These are the two architectures used today. Do you think there is any alternative? Will we keep using them for evermore?

### Laptop: shared memory 

Modern processors are all multicore processors, with many CPU-cores manufactured together on the same physical silicon chip. This is an example of shared-memory architecture because all of the CPU-cores *share* the same memory. Recall the analogy of several workers in an office sharing the same whiteboard. 

However, there are key limitations associated with the shared-memory approach as all of the CPU-cores compete for access to the memory over a shared bus (much like the obvious issues in trying to cram too many workers into the same office). This can be alleviated to some extent by introducing memory caches or putting several processors together in a NUMA architecture, but there is no way we can reach the hundreds of thousands of CPU-cores we need for today’s exaflop supercomputers.

Shared-memory systems are difficult to build but easy to use, and are ideal for laptops and desktops.

### Supercomputer: distributed memory 

The distributed-memory architecture takes many multicore computers and connects them together using a network. Recall the analogy of workers in different offices communicating by telephone. With a sufficiently fast network we can in principle extend this approach to millions of CPU-cores and beyond. 
...
When discussing limits on performance, we have concentrated on discussing memory contention. However, we now know that a distributed-memory parallel supercomputer is made from many individual shared-memory computers. These nodes need to communicate over a network. Hence, network performance is also important. 

### Latency and bandwidth
It’s worth getting some feeling for memory and network performance so you can understand the factors that limit performance in practice. We’ll use the ARCHER system to give us some typical values. The two basic measures are latency and bandwidth:
...
For access to memory (remember: this is different to the cache! - access to cache is faster), the latency (delay between asking for a byte of data and reading it) is around 80 nanoseconds (80 x 10-9 or 80 billionths of a second). On ARCHER2, each node has a bandwidth of around 200 GBytes/second.
...
When looking at the top500 list in the previous part, you may have noticed that many of the world’s largest supercomputers use some kind of accelerator in addition to standard CPUs. A popular accelerator is a General Purpose Graphics Processing Unit, or GPGPU. We briefly touched on this in the introduction, but now we have covered how a modern multicore CPU works we can understand the design of a GPGPU.
