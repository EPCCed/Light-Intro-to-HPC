# Connecting multiple parallel computers 

Because of the difficulty of having very large numbers of CPU-cores in a single shared-memory computer (recall the limitations related to memory access speed and memory capacity) all of today’s supercomputers use the same basic approach to build a very large system: **take lots of separate computers and connect them together with a fast network.**

## Shared Memory Architecture 

Recall this diagram from the previous lesson: 

```{figure} ./images/hero_6d93ece3-84b2-495f-b5c5-0e0f652196ea.png
```

Here we have a single shared-memory computer where all CPU-cores within the processor are connected to the same memory. 

## Distributed Memory Architecture

Now, let's study this diagram of multiple shared-memory computers connected together: 

```{figure} ./images/hero_cb43e824-caf6-43f9-9d5c-85f347dc7373.png
```

Important points: 
1. Each seperate compute is called a **node.** 
2. Each node has it's own memory, totally separate from all other nodes. 
3. Each node runs a separate copy of the operating system. 
4. The only way that two nodes can interact with each other is via communication over the network. 

So, in this diagram, we have 4 nodes which are connected via some network. 

<!-- Do we need to define network? -->

Let's return to the office analogy: a distributed-memory parallel computer is like workers all in separate offices, each with their own personal whiteboard, who can only communicate by phoning each other.
...
We are no longer limited by the hardware and have an excellent recipe for building huge distributed-memory supercomputers! 

But, unfortunately, we are limited by the software. We now have to be able to write a program that can take advantage of all those thousands of CPU-cores and this can be quite challenging in the distributed-memory model.
...
To help you understand the general concepts we have introduced in this part, we’ll now look at a specific supercomputer: **ARCHER2**. ARCHER2 has already been discussed in this course, now let's dive into the details. You will have come across some of this terminology before, but some of it might be new - this is just to give an overview of a real system. 

### Introduction

Archer2 is a HPE Cray EX machine. It contains 750,080 CPU-cores and has a theoretical peak performance of 28 Pflop/s. It is operated by EPCC at the University of Edinburgh on behalf of EPSRC and NERC, and is the major HPC resource for UK research in engineering and in physical and environmental science.
...
The basic processor used in ARCHER2 is the AMD Zen2 (Rome) EPYC 7742 CPU, which has a clock speed of 2.25 Ghz. Each ARCHER2 node has 2 processors, each processor has 64 CPU-cores for a total of 128 CPU-cores per node. All the cores are under the control of a single operating system. The OS is the HPE Cray Linux Environment, which is a specialised version of SUSE Linux.

### Network 

The complete ARCHER2 system contains 5,860 nodes, this means that ARCHER2 is effectively 6,000 seperate computers each running their own copy of Linux! They are connected by the HPE Slingshot interconnect, which has a complicated hierarchical structure specifically designed for supercomputing applications. Each node has two 100 Gb/s network connections, this means each node has a network bandwidth 2048 times faster than what is possible over a typical 100 Mb/s fast broadband connection. 
...
This video uses Wee ARCHIE to explain the parallel computer architecture concepts introduced in this week.
